...
namespace {
    ...
    cl::opt<LibcType>
            Libc("libc",
                 ...
                 cl::values(clEnumValN(NoLibc, "none", "Don't link in a libc"),
                            ...
                            clEnumValN(UcLibc, "uclibc", "Link in uclibc (adapted for klee)")KLEE_LLVM_CL_VAL_END),
                 cl::init(NoLibc));
    ...
    cl::opt<std::string>
            URL("server",
                        cl::desc("Solver server url"),
                        cl::init("http://127.0.0.1"));
    cl::opt<int>
            PORT("port",
                cl::desc("Sover Port"),
                cl::init(5000));
...
void KleeHandler::updateGenerateTestcase() {
    ++m_numGeneratedTests;
}
...
//            modified input argv and run second time.
        ...
        for (unsigned i = 0; i < new_argv.size() + 1; i++) {
            std::string &arg = (i == 0 ? InputFile : new_argv[i - 1]);
            unsigned size = arg.size() + 1;
            char *pArg = new char[size];
        ...
        KleeHandler *handler = new KleeHandler(second_pArgc, second_pArgv);
        Interpreter *interpreter =
                theInterpreter = Interpreter::create(ctx, IOpts, handler);
        handler->setInterpreter(interpreter);
